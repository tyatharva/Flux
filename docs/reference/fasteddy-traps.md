...
("traps §12"). The short forms are the [standing rules](standing-rules.md). This page has
...
Found 2026-08-20. The one that came closest to corrupting a result. A 400-step smoke test with
...
`.` in `inFile`. `restart.nc` leaves `simTime_itRestart` uninitialised. Used on purpose:
...
terrain-following metrics but flat *diagnostic* coordinates in every later dump. It is also the
only mechanism that gives v5.0.1 a spatially varying `z0m`. `bin/prep_restart.py` uses it.
...
the boundary layer 51.3% of cells have `(2/3)e < 1e-6`, so the ratio evaluated to 1e6 there,
`T_L` collapsed, and every particle above `z_i` was pinned at `dt_min`. Median ratio 6.29 where
it should be 1.00.
...
A guard on one side of a division is a bug on the other side. And check the wall clock. Two
runs of the same size that differ 4× in duration indicate a problem.
...
starts. Check the artifact the step was supposed to produce.
...
all ten written parameters against limits read from `hydro_core.c` itself. Grep logs for
`outside limits` beside `CORRUPTED`.

A numerical near-miss is recorded because the fp32 reasoning is wrong. The hydrostatic
integral contains `(1/g)·log(1 + g·dz/theta)`, which looks as if it loses the neutral limit at
...
`frqOutput` to that step (trap 5 otherwise removes the dump). `bin/c2_restart_check.sh` takes
the step as an explicit argument and re-scored the same artifact as PASS, 0 of 23 differing.
When a restart check reports a difference far above the 1e-4 floor, suspect the test's step
...
`bin/pick_seed.py` keeps matching on the peak fraction, because that is the definition
`window_stats` uses for the corpus input `h`. The same dumps re-scored: PASS at +1.87 %/h,
...
the tool (`bin/case_surface.py`, `bin/prep_restart.py`). Change the restart *file*, not the
`.in`. Chaining was retired on 2026-08-26, so the only restart left is seed → target, where the
surface is written on purpose. Check the flux the dump contains, not the flux its `.in`
...
| `bin/seed_stationarity.py` | `--zm 10.0 --k 2` defaults | the manifest's `gate.zm`, `gate.k` | scored Kljun at 10 m and read `σ_w` from 21.4 m |
...
The fix is not "pass a flag". Read the value from the artifact the run will actually use. The
job manifest has `gate: {zm, k}` and `grid: {dx, nz, zceiling, deform, domain_m,
surflayer_z0}`.
...
- **19e.** The mean of a ratio is not the ratio of the means: `L` wrong by 148×.
  ...
  −25.45 m, `z/L` −166 vs −1.12. Every downstream number was finite and plausible. It went
  unnoticed for three near-neutral pairs. Fix: reconstruct the numerator per cell and average
  that. The corrected mean reproduces the grid's own `htFlux.npy` domain mean to four decimals.
...
- **20c.** `window_stats` mixed two surface-flux estimators inside one window. It used the
  `htFlux` variable when a dump had it (full dumps under `ioLPDMfullFrq`) and derived the
  ...
  output mode. Found by the ring, which has no `htFlux`. Derived per cell for every dump now.
...
- **20h.** `keep=True` turned the consumer's drain loop into an unbounded accumulator. Deleting
  ...
  whether that side effect was also enforcing a control-flow invariant.
...
- **21a.** The hand-off removed about 20 GB of disk and moved it to RAM, and every check
  ...
  argument is a rented machine. Deleting the tmpfs file releases the *producer's* backpressure
  and nothing else. Fix: the two passes are fused into one (`WindowAccumulator`. `window_stats()`
  ...
  reduce: the 12.0 GB field cache is the window, random-accessed by a CPU integrator.
...
first minimum and requires the global peak to be inside it. The test is "a second layer with
more energy than the first", not "the first local minimum" (using the first strict minimum moved
...
has both halves of the comparison. `liblpdm.so` is one translation unit built whole-program
...
includes `<cub/cub.cuh>` for the slab means the subsidence forcing uses. The dialect is raised

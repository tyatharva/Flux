...
target, receptor wind from 251.7° for a 270° forcing. That is 18.3° of Ekman backing, 5.2° off
the 23.5° `pick_seed.py` assumed for an unspun seed.
...
deepening. The estimator was. `z_i` as 5% of the running TKE peak follows a peak that falls with
...
boundary-layer average (the column mean divides by the whole 2500 m domain and rises
mechanically as `z_i` rises. The BL-average is 1.50 vs 1.42 across the two neutral rungs where
the column mean differs 44%). The scoring window became 2.0 h (measured over 1.0–2.5 h. Longer
reaches into the cold start). And a limit within 3 SE of its threshold became INDETERMINATE.
The trend was then −0.15 %/h, indistinguishable from zero.
...
neutral rungs are at 8.4 and 10.2 turnovers in 3.0 h and the convective rungs at 19–23.

## `seed_cbl-deep_a000` (16 m, 2026-08-27): locked in, the rung does not fit the domain
...
The direct test of the domain: mode-1 share 19.3–23.1% at `L/z_i` 2.97 (against 53.9–72.0%), peak
wavelength at `L` in 3 of 4 dumps rather than 5 of 5, `r(L/2)` −0.076 to +0.067. By the fifth
pass's own standard (50.2% accepted as footprint-indistinguishable) the domain no longer organises
the thermals. The depth overshoot is a property of the rung, not the domain: 1308 m (fixed) and
...
for. Re-specifying it was flagged, not done. The production library kept `cbl-deep` at a 950 m
...
- Gate on ratios that follow the inertial oscillation, and on a fixed-threshold `z_i`.
...
- The domain width, not height, is what a deep convective rung needs. 30 m on 3660 m is wider
  still.
- Seeds are matched on what they achieved, and a case's inputs are read from its own window, so

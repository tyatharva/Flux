...
0.30% of the domain by area, an 85× enrichment. Kljun on identical cells 150 m, 23.2 ha, 0.894.
...
(`u*` matched to 2.4%). Decomposed: the tower bin is at a more unstable ζ, and corrected for
...
`TKE_BL/u*²`, while the three limits the footprint's geometry depends on were established ok
...
comes with that gap ([limitations](../limitations-and-future-work.md)).
...
the array share 1.52% where the northerly geometry predicts about 25%. Both windows had it.

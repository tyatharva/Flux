...
- Loader, features, model, losses, metrics and the two gates (`bin/test_ml_data.py`,
  `bin/test_ml_model.py`) built against `corpus_cone.h5`. The test split refused by the loader
  and every read logged.
...
- **Phase 2**, Optuna `fno_v2`: 120 trials in 3.4 h, 60 complete, 60 pruned, best #40 at
  ...
  for want of the driver's pruner.
...
- 15:59 phase 1 done; the winner `v_s0.1_seed0` by the stated rule (the loss), with the
...
- Figures on val, five iterations (v1–v5): the showcase (four cases with a per-case metric
  ...
  3DEP contours. Design choices settled on the way: Esri imagery under the footprint panels,

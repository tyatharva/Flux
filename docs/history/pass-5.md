...
it bought and what it cost are both recorded; the configuration was retired two passes later
...
| B4 terrain `dt` | PASS | clean to CFL 1.40; `bin/k0k1_by_slope.py` conditions on slope because the domain-mean check is structurally blind to a few ringing columns |
...
| D2 integral | PASS | LES 0.914 vs Kljun 0.956 on the identical box, converging from below |
...
- **B4's check was structurally blind** (a domain mean cannot see 1.7% of cells).
...
Two convective windows in the same 1952 m box, identical in everything including surface heat
flux (0.1363 K m/s virtual), `z_i` separated by the capping inversion and subsidence alone:
...
swing had been about 370×; here the tower sits inside the array and sees it from every
...
The surface representation was worth 2.76 points (0.90 roughness, 1.86 displacement), measured
where the share is closest to saturation; Kljun predicts +8.3 points on a crosswise direction.
`--raise-topo` was adopted as the physically better treatment.

## What was found that changed the science

1. The lake left the study (0.05% of the box) and it cost nothing at this receptor.
...
4. The `z_i` cap is expensive and biased: excluded deep hours carry 1.51× the heat flux.

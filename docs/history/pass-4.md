...
and one regime added. Every absolute distance is a 30 m-receptor number on the 24 m grid.

## A window is 30 min plus `t_back`, and FastEddy had to change to allow it

The third pass's footprints were 15-minute footprints wearing a 30-minute label: a backward
...
it in full upstream form, so a restartable dump sits at each end of a chain link. Verified on
...
restarts cleanly; cost one 170 MB dump per link against 42.7 MB lean.
...
`bin/conus404_site.py` streams a stratified 45-year hourly sample at the tower cell off the
...
takes it as an input; and the wind rose (S 16.0%, NW 14.5%, W 14.4%, SW 14.3% against N 10.6%)
...
kept climbing (0.747, 0.882, 0.968, 1.031, 1.089 at 300–900 s) while the peak sat at 264 m
...
Kljun on the identical 4464 m box integrates to 0.875, so LES 0.888 against Kljun 0.875 is
agreement to 1.5%, and the shortfall below 1 is the tail outside the box. **Decision:
`t_back = 900 s`, production windows 45 minutes.** Recorded rather than waved through: the
...
a hollow at 300–600 m lifts trajectories over it, and the far lobe at 1080 m sits on rising
open cropland beyond. A difference from Kljun you can point at on the map. **The floor
...
domain.** Three compounding effects: `x50` halves (468 → 202 m); the achieved wind is closer to
north (353° against 335°, Ekman turning 7–13° in a well-mixed CBL against 22–25° neutral), so
the chord is 252 m against 143; and the array carries 1.6× the heat flux. The lake runs the
...
## The sub-grid gate, and what the floor is worth
...
limit and differ in the transition where a 30 m receptor under a 900 m CBL sits.

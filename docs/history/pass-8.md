...
| water in the domain | 8.78% | 13.61% (2026 cells) |
| array in the domain | 0.50% | 0.30% (44 cells) |
...
the cap removed 6.1%. 186² at 24 m gives full containment at +132% (820 → 2150 GPU-h). 122³ at
30 m gives 25% more domain for 3%. A relative claim against Kljun does not need full
containment, and the parity number is what made that defensible: LES 0.874 of its asymptote
against Kljun's 0.867 on identical cells at 2928 m. The acceptance became "the neutral integral
saturates by 2.5 L", not "it is complete". (The ninth pass then found the parity does not
survive the move.)
...
1.3502, 10.0% below the last clean rung. It puts the 5 s cadence, the 300 s seed cadence, a
...
so the physical footprint did not change. What changed is that a 3660 m domain contains it where
a 2928 m domain replaced the lake between 1464 and 1830 m east with a periodic copy of its own
...
## The LES passes its fields to the LPDM in RAM
...
- **The ring holds a full window (541 slots, 6 fields, 12.0 GB), not `t_back` (180 slots,
  ...
  would force the floor onto partial-window statistics. That would be an estimator change
  disguised as a plumbing change. A slot is 122 × 123 × 123 wrap-padded, 3.691 MB per field at
  fp16.
...
1800 s adjustment plus two 2700 s windows in one invocation, with window 2's releases beginning

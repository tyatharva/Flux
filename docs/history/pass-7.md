...
the receptor was raised instead: **30 m on 122³ at 24 m**, a 2928 m domain. The real tower stays
at 10 m. The emulator now predicts, by design, a footprint the instrument does not measure.
...
| B5, rotation | PASS at the fp32 floor. Four turns are the identity bit-for-bit |
| taper | pad 10, real terrain to 1224 m |

**The lake is back and Gate A1 fails in one regime.** Water is 8.78% of the domain (1307 cells).
The worst-case footprint water share is 17.45% in very stable easterly conditions, which the
corpus does not contain, and 7.38% over the regimes it does. Both numbers are recorded. **The
array leaves the footprint for E/W winds.** Kljun at 30 m gives N 30.7%, E 0.04% (against 80.6%
and 29.9% at 10 m). "This tower measures the array in every wind direction" is a 10 m statement.
The `z_i` band moved to 300–1250 m (floor `10 z_m`, ceiling the lower of `L ≥ 2 z_i` and the
domain height). Day coverage went 75.0% → 80.4%, 1370 → 1469 cases, but the deep exclusion still
has 2.33× the heat flux of the accepted hours.
...
3. **The tower `σ_w` check translated from 10 to 30 m** (`bin/sigma_w_tower.py`): invert
   ...
   the tower file has no wind speed, and the stable branch is an upper bound.
...
vs 1.095), which establishes that the window (900 s of a layer 1800 s old) is the cause, not
the port. Not done: the in-FastEddy hook, which the [eighth pass](pass-8.md) built as the
in-process hand-off.
...
`nbl-deep` with the accelerator ran to the 3.0 sim-h ceiling at 0.483 GPU-h/sim-h, with every
...
53.9–72.0% in the 1952 m domain. `r(L/2)` zero), the depth overshoot unchanged at 1308 m, above
...
**|Δpeak| = 144 m, six times the larger floor, with the ordering matching Kljun's**, max/min 2.00×
...
integral (1.463 → 1.634) moves too. The 144 m separation is the LES's.

Settled at no extra cost: `t_back` = 600 s captures 99.6–100% of the 900 s integral at a 30 m
receptor, and the peak is at its final value from 150 s. Production kept 900 s. And **the
integral's departure from the asymptote follows the mean vertical velocity at the receptor with
...
What the test did not establish. Both targets came from the same seed at different rotations

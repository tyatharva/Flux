...
1. **CUDA-aware MPI is not required.** FastEddy passes raw device pointers to `MPI_Isend` /
   `Irecv` under `hydroBCs = 2`, which looks like it demands it. A from-source CUDA-aware
...
8.15 h wall (over the plan's 4 h threshold), and the output volume, with no vertical-subset
...
exactly 30.000 m on cell centre `k = 1`. Deliberately not the science grid.
...
- **`eps` is FastEddy's own**, `c_e e^{3/2}/l` with `l = min(0.76√e/N, Δ)`, read out of
...
- **The near field was missing.** Inside 190 m the LES had 4.3% of its influence against
  ...
  A 4× sweep in `C0` moved the peak by one cell; a seed change by zero. Not tunable.
...
- **The restart file carries the surface, and that is a trap and a lever**
...
fraction measured alongside, on the flat window: uncapped, the integral sailed past 1 exactly
...
rotation) was implemented and found worth 2%, not 64%; kept because it is the frame the
...
marginal for the centroid, the lever is sampling time within a run rather than number of
runs, and a production window needs `t_back` plus the sampling time.
...
predicted 35.5% from the band it occupies. The integrals saturated (wrap cap holding), the
westerly settling 30–40% high where the receptor sits in mean ascent (advective
non-closure, `w̄ = +0.064 m/s`, concentration integral 18.5 against 8–10) and the other two
about 18% low because the footprint barely fit the 4380 m box (80% source area to 3810–3870 m).
Terrain footprints were noisier for a measured reason: `t_back` ate half of an 1800 s window,

...
84.1% for due north and 37.8% for east or west, and every alternative the seed reached was on
the E/W axis. This one, 7° off north at about 79%, had roughly double the array signal.
...
The window's `z0m` matched the case grid to 1.5e-9 and `htFlux` to 3.3e-8, and array `z0` 0.250 vs
0.100 was asserted on the run. 450 × 700 = 315,000 particles, 704,133 touchdowns. Release period
...
Array share 68.44% unwrapped (67.23% folded) against 1.03% of the domain by area, a 66× enrichment.
...
so it is not a wraparound runaway. The receptor is in a local depression at −5.43 m with a
model-frame mean `w` of −0.0976 m/s. The double rotation removes 96.2% of it. Over sloping
ground the turbulent flux is not the surface flux. Closure: floor factor 1.000 at the
...
the 80th percentile, 3.05× the median, and that is expected. The file has no wind speed, the
...
0.472, +12%. The 12% against similarity is the more informative number.
...
weaker than the seed's (+8 K/100 m). Neither makes the pair wrong. Inputs are read from the LES
window, so the case lands at (`h` 559, `wdir` 331.2°) instead of (510, 353.0°). The library's
spacing gives less convergence on direction and more on depth than designed, which is why the
...
comparatively insensitive to it. Two guards followed. `h` is refused if it reaches the top of the column,
...
profile whose wave layer had more energy than the boundary layer (traps §22).

...
August 2026. Grid 186 × 186 × 122 at `dx = 24 m` (a 4464 m domain), `dz_sfc = 8.558 m`, receptor
...
There is no rotated GIS anywhere. `bin/prep_surface.py` builds terrain and land cover once on
a fixed north-up EPSG:3071 grid, and direction is changed by rotating the flow. The class of
bug that put the array at a fixed upwind distance became structurally impossible.
...
- Land cover: ESA WorldCover v200 (2021), 10 m, mode-resampled (a class map is never averaged).
  ...
  per annulus. WorldCover was adopted because it is authoritative and has every class.
- **The array, corrected.** The tower is *inside* it, 60 m east and west, 250 m north, 100 m
  ...
  it is an override. Because the tower is inside it, the upwind reach depends on direction
...
`z/Δ ≈ 1.5` the eddies that carry `w` are at or below the filter scale. Low `σ_w` makes
...
not adopted. It was a constant fitted to one case at one height with no rule for transferring
it. The two fix the peak identically and differ only on the tail, so **the peak error is a
...
only. 77 GB per window became about 19 GB. CF packing was chosen over IEEE half because it is
...
| 5 | sub-grid fraction < 40% | FAIL at about 80%, but the quantity it stands in for was diagnosed and largely corrected |
...
vs 175× measured, wN/wS 3.0× vs 6.6×, wS/wE 32× vs 26.5×. The ordering is exact, the magnitudes
within a factor of two, and the excess is in the direction the LES's own near-field deficit
requires. Wraparound was ruled out as the source (the far field has at most 2.3%, negative for
wE and wW).

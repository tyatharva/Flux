...
The host Python has no scipy, h5py or netCDF4 on purpose; every analysis script runs in-image.
...
Held identical between them, so the toolkit is the only variable: Ubuntu 22.04, OpenMPI
4.1.2, NetCDF 4.8.1, HDF5, gcc 11.4. The base image is pinned by digest. The upgrade's effect
on the physics is 0.97–1.12× the model's own run-to-run floor
...
`IMAGE_PROVENANCE.txt` inside a deployment image records what was installed. Herbie is
pinned to a floor because its archive source list changes as NOAA moves buckets. `eccodes`
comes from the pip wheel and the apt package is deliberately not installed: the two disagree
on definitions paths.
...
runs a project Python script the same way (`docker/pyrun.sh bin/test_bl_depth.py`), and
`docker/run_case.sh` runs one FastEddy case with the concurrency guard, the `.in` line-length
check, the log grep and the `k0/k1` check. All three branch on `FLUX_NATIVE=1`, which the
deployment image sets, to run directly instead of shelling out to `docker run`.

Two environment settings in the image are load-bearing. `OMPI_MCA_plm=isolated`, because
OpenMPI's default launcher hunts for an ssh binary a single-node container has no use for.
`CUDA_HOME`, `MPI_ROOT`, `NETCDF_ROOT` supply the include and library paths FastEddy's Makefile
expects NCAR's module wrapper to inject.
...
Classic builder (`DOCKER_BUILDKIT=0`): this machine has BuildKit without the buildx component,
and the classic builder reads exactly one `.dockerignore`, which is why there is one file for
both Dockerfiles. The build refuses a dirty tree unless `FLUX_ALLOW_DIRTY=1`. It fetches
FastEddy itself, compiles it with `-std=c++17` for seven architectures, builds `liblpdm.so`,
and then asserts on the artifacts: the SASS list of both binaries, the absence of PTX in
FastEddy and its presence in `liblpdm.so`, the nine-warning baseline, 30 seed restarts of
...
No Docker image carries torch. Training ran on the RTX 4080: an FNO seed in 7–13 min, a CFM
seed in 19–25 min, one process at 100% utilisation and about 3 GB; four concurrent runs buy
1.1× throughput.

## The host

The workstation runs Linux with an RTX 4080 and Docker with the NVIDIA container toolkit.
The host-side toolchain that was installed alongside (CUDA toolkit, gcc-13 and gfortran-13
from the toolchain PPA, OpenMPI, HDF5/NetCDF/PnetCDF, ParallelIO, an LLVM/Flang build) was for
other atmospheric-model work on the same machine and is not what any result here depends on;
the reproducible environments are the two Dockerfiles. Two of those host choices are wrong for
FastEddy and are why the images exist: Ubuntu 22.04's `nvidia-cuda-toolkit` is CUDA 11.5,
which predates `sm_89`, and nvcc 11.8 accepts gcc ≤ 11, not gcc-13.
...
| `docker/turb_alive.py` | image | is there any turbulence at all? Runs everywhere `k0/k1` runs |

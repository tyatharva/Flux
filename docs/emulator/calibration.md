...
The CFM follow-up of 2026-09-02, about 3 hours of wall time after the first CFM run. Code:
`ml_cfm/crps.py`, `ml_cfm/tailthresh.py`, `ml_cfm/calibrate.py`, `ml_cfm/sample_count.py`;
gate `bin/test_cfm.py` (26 checks). Numbers in `results/ml_cfm/calib/` and
`results/ml_cfm/tail/`.

## The answers

1. **The under-dispersion where the array is in view is a mis-scaling, not a mis-shaping,
   mostly.** A single global temperature τ = 1.19 on the sample spread, fitted on val and
   cross-fitted by record parity, brings the array-share coverage into the ±2 sd band on all
   three groups. The in-view group lands on the band's lower edge with z sd still 1.31, so
...
2. **CRPS training does not fix it.** Fine-tuning the flow on the pixelwise fair CRPS leaves
   the array-share coverage where it was. What it buys is a 2-step sampler as good as the
   16-step ODE.
...
record, mean over the group; spread/skill = rms sample sd × √((S+1)/S) / rmse of the sample
...
the t-augmentation that keeps the flow loss honest does not carry over to an objective that
...
about 4000 tail cells of the cone where the spread is already right; the 44 array cells carry
no weight in it.

Two conditional runs, triggered by the plan's rule (in-view cover90 < 0.81 after the
...
0.470–0.483 for seeds 1–3). More samples are not a lever.
...
S = 4 / 4-step variant 4× that again. Evaluator: about 2 min per variant at S = 64.

...
## Not generative, and the real reason
...
- **Model** (`ml/model.py`): lift → `depth` × [spectral convolution on the lowest `modes`² of
  ...
  (`bin/test_ml_model.py`). FiLM is used because the scalars are global quantities. A spatial
  encoding would be six constant planes at 128² times the cost.
...
1e-3 / 1e-4, cosine, batch 16, gradient clip 5, σ = 0.1. Selection and early stopping are on
the val MSE of the 4-sample (later 16-step Euler) mean every 5 epochs, the same number the FNO
was selected on. There is no adversarial term. Samples have the LES's texture without one.
...
h5py 3.16 and optuna 4.9. The spec is `ml/environment.yml`. No Docker image contains torch.
The emulator is the one analysis that runs on the host.

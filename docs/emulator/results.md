...
test split. The CFM's sample mean edges the FNO on the centroid and the 1-D shape and ties it
elsewhere, with a tenth of the parameters, and it is the only one that gives an error bar.

## The headline: test split, 294 records of 2025, frozen recipe

`results/ml_cfm/final_recipe/metrics_test.md`. Kljun raw; FNO = mean of seeds 0–3, cut at its
own 99.5% source area; CFM = physical-space mean of 80 samples (4 seeds × 20, τ = 1), cut
likewise; LES positive-only. Errors are RMSE over records; scores are means.
...
Metric definitions: peak distance = |upwind peak-distance difference| of the
crosswind-integrated footprint; centroid = distance between the mass centroids; integral =
|integral difference|; overlap80 = Jaccard of the two 80% source areas; rel. L2 =
‖model − LES‖ / ‖LES‖ on the 122² interior; sliced W1 = mean over 64 directions of the 1-D
Wasserstein-1 between the unit-mass positive parts; JS distance = √(Jensen–Shannon divergence)
in bits between the unit-mass positive parts; MS-SSIM = 5-scale SSIM on the log10 grid with
...
where the site signal lives. Floors from `results/les_realisation_spread.txt`, the fourth
...
recommended model: the lower-variance estimate of the same conditional mean.
...
W 0.39, NW 0.51: below 1 everywhere. By stability tercile the advantage grows toward the
...
sits nearer any one sample than samples sit to each other.
...
Four of five seeds land on 1.17e-4, the FNO's number, with 2.95 M parameters against 28.4 M
...
The CFM mean beats Kljun on every metric and essentially ties the FNO: significantly better on
the centroid, array share, 1-D shape and SSIM and never significantly worse. Composite 0.492
against 0.526 (ratio 0.948), below 1 in every octant except E (1.09, n = 10). On the 42
array-in-view records the two are indistinguishable on every metric (p 0.18–0.89) and both
beat Kljun on the array share (3.48 and 3.51 pp against 5.00 pp). Integral against the
...
centroid, so the sampled spread is the size of the realisation floor on the overlap, shape,
L2 and centroid, and about two-thirds of it on the array share where the array is in view.
...
FNO 0.0017. Samples carry the LES's texture; the mean is as smooth as the FNO.
...
The isolated low-level cells carry no mass the production metrics see.

## Cost

FNO: 28.4 M parameters, 436–783 s per seed at three concurrent runs on the RTX 4080. CFM:
2.95 M parameters, 19–25 min per seed; sampling at batch 64, Euler 16 steps, 17–24 ms per
record per sample, so 32 samples cost about 0.6 s per record and the 160-sample val set 131 s
per seed. Euler 4 is four times cheaper at the same quality.
...
3. Only about 15% of records carry the array signal, so the northerly breakout is the one that
   matters.

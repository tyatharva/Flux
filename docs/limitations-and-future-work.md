...
1. **The model receptor is 30 m. The instrument is at 10 m.** The emulator predicts a
   footprint the physical tower does not measure. This was a deliberate resolution choice,
   not a correction: at 10 m the footprint peak did not respond to meteorology (48 m in all
   three targets, max/min 1.00×) because the near field was closure output, not LES output.
   Tower comparisons go through a Monin-Obukhov (MOST) translation whose stable branch is an
   upper bound.
2. **Only about 15% of the corpus carries the array signal**, and the wind rose is skewed
   away from the directions that do. The array is in the footprint essentially only for
...
3. **Gate A1 (water share) fails.** 11.58% worst case over corpus regimes against a 10%
   threshold. It is the site, not the box: Kljun's `x90` barely moved between domains (1665 vs
   1615 m), so the physical footprint is the same. What changed is that a 3660 m box holds it
   where a 2928 m box was replacing the lake with a periodic re-sample of its own land. The
   old PASS was truncation. E and NE are about 20% of the rose and carry 6–12% water; the rest
   about 0%.
...
7. **Every seed is DRIFTING or INDETERMINATE on at least one limit, and that is the normal
   ...
   the run is short. The neutral rungs are genuinely short at 2.0 simulated hours.
8. **The GPU LPDM is validated but is not the production integrator**, so host memory floors
   at the 12.0 GB fp16 field cache rather than at one or two snapshots.
9. **The training target is `corpus_cone.h5`, and the wraparound it crops is a boundary
   ...
   candidate that fits (departure tracks `w̄` at the receptor with the right sign: subsidence
   1.497×, updraft 0.916×), and testing it needs `w̄` per record, which the corpus does not
   carry.
10. **Seed grouping in the split is not settled.** `bin/seed_leakage.py` found no fingerprint
    ...
    floors), but with n = 1 same-seed pair there it is weak evidence, not a licence to drop
...
12. **The corpus has gaps.** 166 days failed. Six months are empty: 2021-12, 2023-07, 2023-10,
    ...
    `/dev/shm`; root cause unresolved, the LES logs are gone). Verified not to be an
    input-space hole: 84–93% of a missing month's cases fall inside the retained months'
    p5–p95 on every scalar.
...
- **Top up the corpus.** The failed days are named in the machine manifests
  ...
  reproduces exactly the cases that would have been there. One command per machine; see
...
- **Carry `w̄` at the receptor in every record**, so the advection non-closure can be tested
...
- **Return the scored series from seed runs, not only the verdicts fitted to it.** Owed since
  the seed library was spun.
...
- **Stable cases need a finer grid.** About `dx = 6 m` at this site, 17× the cells. Not
  affordable today; the emulator should say "undefined" rather than extrapolate there.
- **Compare against the tower.** The MOST translation from 30 m to 10 m and its stable-branch
  upper bound are described but not exercised against measured fluxes.

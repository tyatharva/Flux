...
are the same nine figures on the same axes, so the two directories diff by eye.
...
That is deliberate, and it is the first thing to check by eye.
...
axis and independently, so a trajectory running more than one domain length reappears
...
them beside `corpus/FLAGGED.tsv`, the record of what the pipeline actually said:
...
again past `q ≈ 11`). The footprint is below `q ≈ 5`, the wrap above `q ≈ 11`, and `k` sits
between them, which is why removed mass moves only 0.4 percentage points across a factor of
four in `k`. Full derivation: [cone mask](../history/cone-mask.md).

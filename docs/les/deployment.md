...
Two campaigns share one image, one driver and one SSH workflow. They were run weeks apart.
...
made it after the machine is gone.
...
The image has no `ENTRYPOINT`. It is explicitly cleared, because the nvidia base image's is
inherited otherwise. Vast's SSH launch mode overrides entrypoints, and the previous one printed
help and exited, which on Vast is an instance that comes up and dies. `CMD` is `sleep infinity`.
...
| disk | 100 GB (image 13.1 GB + 30 seeds × 24 dumps × 73.3 MB = 54 GB in `/out/work` + 2.2 GB deliverables + about 6 GB scratch = about 75 GB. 40 GB is enough with `--prune-dumps`) |
| GPU | 16 × RTX 5090 |
...
CUDA 13.0.1 binary. NVIDIA's minimum Linux driver for 13.0 is 580.65.06. A 5090 machine with a
570 driver will not run it, even though the binary contains `sm_120` SASS. The toolkit major
...
GPUs, not GeForce. If only 570 machines exist, rebuild on CUDA 12.8 (`FE_GENCODE` unchanged.
12.8 also targets `sm_120`).

**RAM, measured on the image.** The LES phase peaks at 0.82 GB container RSS (FastEddy itself
0.49 GB over 2 processes). The acceptance battery peaks at 1.51 GB. The 16-way worst case is
about 24 GB. The 12.45 GB figure elsewhere is a *corpus case* (the LPDM's 12.0 GB field cache).
A seed runs no LPDM. Sizing the seed machine on the case number over-rents by 4×. Sizing the
corpus machine on the seed number makes it swap until it is unusable.
...
dropped), then distributes all 30 seeds across the cards. The sweep affects performance only.
The one reduction that accumulates is templated on compile-time constants.
...
It is resumable. Re-running the same command skips seeds whose work directory has a
...
### What is written to `/out`, and getting it back
...
max, total, from paired values only, naming any seed excluded for lack of one), GPU-h per
...
timeline. Every job is recorded exactly once. A worker took a second job after its first (14
hand-offs, at the 3 s stub duration exactly: 3.1 s, 6.2 s, 9.3 s). The intentionally failed
jobs failed and released their workers. Peak concurrency never exceeded the workers. The run
ended only when the queue was empty. No stub was counted as an accepted seed.
...
Two more facts that are easy to miss. An out-of-range `CUDA_VISIBLE_DEVICES` fails loudly at
...
random-accessed by a CPU integrator). Eight concurrent cases could approach 100 GB, and a
machine that starts swapping does not fail. It runs several times slower with nothing in the
output to say why. Rent ≥ 256 GB for the first machine and read the early report before
renting the other seven.
...
`nat` is 91% of a case. The only parameter that would reduce it is `nlev`, a sounding-fit
input, which is left alone. The screen reads only `hpbl`, `shtfl` and `dz_i/dt`. The 10 m wind
is fetched once, on the accepted hour (before that change a candidate cost 9.19 MB). An
accepted day costs about 8 fetches and an exhausted one 26, so the screening term is dominated
by days that yield nothing: 210 MB per accepted day, 121 MB per missing day, **about 47 GB per
machine**, 374 GB across eight. The network is 2.4 h serial per machine against 9.7–14.8 h of
compute: 2–3% of wall time, about 1.1 MB/s sustained, with brief 61 MB/s peaks if all eight
workers fetch a sounding at once.
...
`--machine N` (0–7) is the only thing that differs between the eight machines. `corpus_progress`
...
`|dz_i/dt| < 15 %/h`) and spend it either way. An exhausted pool is a missing day with a reason.
...
a month's cost is its accepted days, which depends on the weather. Measured on the stubbed dry
...
**The early report**, after 5 cases, is printed and put in `progress.json`: GPU-h per case as
*occupancy* (wall clock × 1 GPU, which is what the rental bills), peak container RSS,
`MemAvailable` low-water mark, swap peak, projected finish. It warns clearly if `MemAvailable`
falls below 12% of total or swap is touched, and prints a `!!!!` block if the projection
exceeds `--max-hours` (default 12). `--abort-on-overrun` stops instead.
...
with the byte-shuffle filter. 1.32× compression without shuffle, 1.65× with, and gzip-9 gains
...
that looks like), a duplicate `run_id` (two machines ran the same `--machine`), a `meta.stub`
record, a count mismatch between disk and the manifests, reported per machine, non-finite
cells, wrong shape, non-zero pad and wrong `dx`.
...
resumed while the manifest still says what each day *is*. There is one record per accepted day
and no others. Every record passes the schema check *and* is refused as a corpus record because
it has `meta.stub`. The stub does not return instantly by design (a few ms, varied per case
...
- Seed selection uses the whole library (`ALLOW_DRIFTING=any`). Every pair has
  `seed.gate_state`.
- `--only-month` refuses a month this machine does not own, otherwise two machines would
  generate the same days.

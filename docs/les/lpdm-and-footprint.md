...
- `w_release` is the vertical velocity the trajectory had at the receptor, the resolved LES
  value plus a sub-grid draw. Trajectories that arrive with `w > 0` came from below and carry
...
- **Trajectories are retired at one streamwise domain length** (`max_disp = fs.Lx` = 3660 m).
  ...
  later touchdowns are the same eddies counted again. Uncapped, the flat-control integral went
...
- **The integral asymptotes to `1 − z_m/z_i`, not to 1** (Steinfeld et al. 2008, after Horst &
  Weil 1992). The fraction `z_m/z_i` of the column is below the receptor and its flux never
  crosses it. At 30 m in an 800 m layer that is 3.75%. Departure from the asymptote follows `w̄`
  at the receptor with the right sign (subsidence 1.497×, updraft 0.916× on two cases of
  opposite sign). This is the advective non-closure that makes eddy covariance hard over complex
  terrain. An integral that crosses 1 and keeps climbing cannot be truncation and is always a
  model inconsistency. A saturating integral above 1 is `w̄` times the concentration integral.
...
target that a wrong constant cannot match by accident. Measured 0.530 and 0.753 against 0.600
and 0.778, within their standard errors.
...
it looks like a plausible footprint). Forward and backward well-mixed tests agreeing is what
confirms the sign.

## Fields and the sub-grid model

`lpdm/fields.py` loads the window into a time-indexed **float16 cache** (12.0 GB for a 541-dump
production window. The 4-D linear interpolation is written by hand because
`scipy.ndimage.map_coordinates` refuses float16, and it matches it to float32 roundoff). fp16
was verified harmless on real fields: peak identical, centroid +19 m, integral +0.015, overlap
75.7% against a 59% half-vs-half floor.
...
`N² > 0` else `Δ`, `Δ = (dx dy dz J)^{1/3}`, `c_e = 0.93`, read from `cuda_sgstkeDevice.cu` and
...
Below the lowest LES level the LES has no information. The horizontal wind is continued by
...
**The LPDM is forked into 16 fixed chunks with per-chunk seeds**, so the worker count is only a
performance setting and cannot change a result (asserted bit-identical, 1 worker vs 12,
...
At `z/Δ = 1.52` the eddies that carry `w` are at or below the filter scale, so the LES
...
surface-layer 1.25, and peak +86%). The fix is a **MOST-anchored floor** (`lpdm/sgs_floor.py:most_floor`,
in one place, and the gates import it):

- The target is `σ_w = 1.25 u* φ_w(z/L)` with `φ_w = (1 − 3z/L)^{1/3}` unstable (Panofsky et al.
  1977) and `1 + 0.2 z/L` stable. `φ_w(0) = 1`, so neutral results are unchanged.
...
- **It is weighted by the sub-grid fraction**: `sc_eff = 1 + (sc − 1)·f_sgs`, `f_sgs = (2/3)e/(ww + (2/3)e)`,
  with monotonicity re-imposed afterwards.
- **`eps` is scaled with `σ²`** so `T_L = 2σ²/(C0·eps)` is preserved, with both sides of the
  ratio floored at the same value. (A clip on one side inflated `eps` a millionfold above the
  boundary layer and pinned every particle at `dt_min`. The only symptom was a 4× slowdown.)
- The drift includes the floor's gradient: `sc·dσ²/dz + (2/3)e·dsc/dz`, where the second term is
...
unmodified model, while supplying 3.49× where the variance is sub-grid.

**The near field is closure-dominated, and that is a number.** The floor adds +8.40 points
...
+18.46 points. The sub-grid fraction at the receptor is 52.5% convective and 86.4% neutral
(57.7–59.3% and 90.4% measured on the 30 m targets). Reaching 40% at a 10 m receptor would
need `dx ≈ 3–4 m`. The choice of anchor (Panofsky surface-layer, adopted, against Lenschow
mixed-layer) moves the crosswind-integrated shape by 46–66% L1 against a 38% sampling floor.
Quote that band with any near-field number. **Quote the compaction ratio with its closure**:
floor off 0.57× (convective broader), floor on 1.33× (more compact). Both pass Gate D1, because
well-mixedness tests self-consistency, not whether `σ_w` has the right magnitude.

## `h`, the boundary-layer depth

`lpdm/les_stats.py:bl_depth` finds the surface-attached layer by walking up from the ground on a
smoothed resolved-TKE profile to its first minimum, and requires the column's global peak to be
inside it. The test is "a second layer with more energy than the first", not "the first local
minimum". Using the first strict minimum moved `h` on 15 of 47 stored profiles by up to 331 m on
profiles with no wave layer. What identifies a wave layer is that it has more resolved TKE
...
19 GB of window dumps. Host residency cannot fall below the 12.0 GB field cache because the CPU
integrator random-accesses all of it. Details and the eight bugs it cost are in traps §20–21 and
the [eighth](../history/pass-8.md) and [ninth](../history/pass-9.md) passes.
...
validated against it and is known to be 1.25× too wide in `σ_y` at `|L| > 5000`

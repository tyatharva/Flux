...
restarts from. A seed exists only to delete the spin-up: a case restarts from the nearest
seed, adjusts for 30 minutes under its own sounding's forcing, then samples for 30 minutes.
Seeds are never training data. Specs and verdicts live in `seeds/`; the 30 restart files
...
The axes are sized by what 30 minutes cannot adjust, which is the only criterion for a state
whose purpose is to be adjusted away. Every number measured on this project's own runs:
...
The rungs are coupled, not a product: a 150 m stable layer cannot carry a 12 m/s geostrophic
wind, so `G` belongs to the rung. They walk the site's real joint distribution as CONUS404
measures it at this tower (`z_i` p25/p50/p75 = 267/493/835 m; `w'θ'` −0.006/+0.015/+0.076
K m/s; `U(30 m)` 3.9/5.2/6.8 m/s):
...
lapse of 0.004 K/m: the `z_i` control, not a profile to be matched. A stable rung (`sbl`) was
built twice and collapsed twice; the cause is resolution ([stable regime](../history/stable-regime.md)).
...
it), so the worst case on the dominant skill axis was 25–35°. Denser angles were the honest
fix; smarter projection was not. Cost: 30 seeds instead of 15, about 2.5% of the corpus.

The seed's own drift at freeze time is projected forward by `bin/pick_seed.py`, which removes
the mean of the excursion and leaves its scatter. Ekman backing per rung, measured off the
library: cbl-deep 4.2°, cbl-mid 5.0°, cbl-shallow 6.4°, nbl-deep 14.3°, nbl-shallow 19.6°
(n = 6 each); by regime convective 5.2°, neutral 16.9°; nominal 23.5° neutral, 22.1°
convective.
...
against unchained at 0.89–1.08× the run-to-run reproducibility floor, so nothing is lost by
removing it, and with it went the 1-hour segment cap. A stationarity dump every 300 s.
...
Kljun `x_peak`, Kljun `x90`). `TKE_BL/u*²` and `z_i` are deliberately not in the stop
criterion: they decorrelate on the eddy turnover rather than on the dump interval, `n_eff`
...
`surflayer_z0` is read off the grid (`data/grid30_raised/z0m.npy`, geometric mean 0.0615 m),
not a constant: an earlier hardcoded 0.1435 m was the 16 m map's value and would have spun
every seed up over the wrong surface. The gate's receptor (`zm`, `k`) is in the manifest and
passed explicitly; scored with defaults a 30 m library would evaluate Kljun's `x_peak` at the
wrong height and read `σ_w` off the wrong level, and every number would still print.
...
the streamwise footprint shape, and both of its terms ride the oscillation together; so do
`x_peak` and `x90`.

`z_i` is the one exception: a length, with no `u*` to cancel. It is made immune by how it is
measured: the gate uses a fixed 0.01 m²/s² threshold, while `window_stats` produces the
...
(`z_i` only lands on model levels), so the gate prints the distinct-level count and span
beside the trend.

The seven limits, scored on the last 1.5 h by `bin/seed_stationarity.py` (the single
definition; nothing restates them):
...
Each trend is reported with an AR(1)-corrected standard error and `n_eff`, and the verdict is
INDETERMINATE rather than PASS or FAIL when the threshold sits within 3 SE of the measurement.
...
Physics orders the way it must: `w_rms` runs 0.28–0.74 m/s on the convective rungs against
0.07–0.20 neutral, and within a rung the six base angles cluster tightly (`U` spreads under
0.05 m/s). The machine: 16 × RTX 5090 (`sm_120`, 32,607 MiB each); peak VRAM 904 MiB per
seed (2.8%); peak container RSS 58.9 GiB, summed FastEddy RSS 8.98 GiB over 32 processes.
The work queue behaved as a work queue: 16 of 16 workers used, 14 took a second job, peak
concurrency exactly 16, zero failures.
...
"repeat noise 0.00%" is quantisation. Worst case if the choice is wrong: 1.7%.
...
Kljun `x_peak` 2, `U/u*` 1. The drift is a rung-wide property: mean gated `TKE_BL/u*²` trend
cbl-deep +3.5 %/h (5/6 accepted under the strict gate), cbl-mid +12.5 (2/6), cbl-shallow
+22.5 (0/6), nbl-deep +19.4 (3/6), nbl-shallow +35.9 (1/6).

**The accept/refuse split tracked the standard error, not the trend.** INDETERMINATE within
3 SE is correct and deliberate, but at these magnitudes the better-measured seed is the one
...
[standing rule 3](../reference/standing-rules.md). The absolute `TKE_BL` trend is recoverable
from what was returned by two routes with disjoint inputs, `trend(TKE_BL) = trend(TKE_BL/u*²)
+ 2·trend(u*)` and `trend(TKE_BL) = trend(domain TKE) − trend(z_i)`, whose agreement is the
...
entrains upward. Every other gated limit is a ratio whose numerator rides the oscillation
with `u*` and cancels it; `TKE_BL` is an energy and nothing cancels. Two conclusions pointing
opposite ways: the convective rungs were not drifting (cbl-shallow's absolute BL TKE is flat
while the gate reports +22.5), and the neutral rungs genuinely are still spinning up
(nbl-shallow rising at +29.4 %/h with both routes agreeing). A 2.0 sim-h ceiling is short for
the neutral half, and that is a real limitation of the library.

## Decision: selection uses the whole library

Since 2026-08-31 `bin/pick_seed.py` ranks all 30; `--allow-drifting` defaults to `any`,
...
and the neutral pick for `case_2023112120` from cost 0.983 with a 14.5° direction gap and the
half-spacing warning firing, to 0.216 with a 1.3° gap and no warning: 4.6× in cost and 11× in
direction gap.

Two defects found and fixed on the way: `run_seed.sh` returned exit 1 for all thirty seeds
(no seed had a clean PASS; now exit 0 when the run produced a seed, `SEED_STRICT_EXIT=1`
restores the old signal), and `n_accepted` in the machine manifest was the count of neutral
seeds (a rebound variable name; the log said 11, the JSON 12).
...
repo root is discovered from the script's own location. It needs a GPU the FastEddy binary
carries SASS for (about 0.65 GB of VRAM measured at 122³; the manifests' 1.6 GB is an
unverified literal), Docker with the NVIDIA toolkit, and about 1.8 GB of disk while running.
About 58 min of wall at the ceiling. **Not resumable**: a killed job costs the whole run;
...
look like a stall. What comes back under `return/`: `seed_restart.nc`, `stationarity.json`
and `.txt`, `manifest.json` with `achieved`, `acceptance.txt`, `seed_report.json`,
`turb_alive.json`, `rotation_check.json`, `direction_drift.txt`, `budget.json`. A job whose
gate fails still returns its artifacts.
...
Seed runs should return the scored series, not only the verdicts and trends fitted to it: a
few kB per seed, and it would make the rescore above a measurement rather than a
reconstruction. The gated form of `TKE_BL/u*²` should be reconsidered now that it is known to
be driven by `u*` and the averaging depth (not changed on the same pass that reinterpreted
its output). The neutral rungs would benefit from +1.0 sim-h (about +2.3 GPU-h for 12 seeds).
Nothing has run the ring or the LPDM on Blackwell; the seeds exercise FastEddy only.

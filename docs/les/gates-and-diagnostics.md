...
Every stage of the pipeline asserts on the artifact it produced, on the quantity and not on the
presence of a file, never on an exit code and never on the configuration handed in. The
[standing rules](../reference/standing-rules.md) are the distilled lessons; this page is the
list of checks, what each asks, and what it cannot see.
...
| IO-registered fields read back | `bin/run_seed.sh`, `bin/prep_restart.py` | the `htFlux`, `z0m`, `topoPos` the dump carries, against what was asked | |
...
Kljun `x_peak` 1.0, `x90` 1.0), scored on the last 1.5 h with an AR(1)-corrected standard error
and `n_eff` per trend, returning INDETERMINATE when the threshold sits within 3 SE. Every limit
but `z_i` is a ratio that rides the inertial oscillation in both terms; the gate prints the
distinct-level count and span beside the `z_i` trend because a line through a staircase reports
the staircase. `bin/seed_watch.sh` scores the trailing window every 30 simulated minutes and stops
the run when the oscillation-immune limits are in band.
...
(when the seed would have stopped). Its exit status is 0 when the run produced a seed and the
verdict lives in `stationarity.json`.

## On a corpus case

`bin/run_corpus_case.sh` asserts the artifact of each of its eight stages. Stage 7c refuses a
...
G2b and G3b are flags, not exclusions: 231 records carry one ([dataset](../corpus/dataset.md)).
...
a count mismatch per machine, or a record off a retired grid.

## The LPDM gates

- **D1 well-mixed** (`bin/stage4_wellmixed.py`, `lpdm/wellmixed.py`): particles released
  uniformly through a deep column, integrated backward and forward, the concentration profile
  scored against uniform below `z_i` (rms against the 5.48% counting-noise floor; the lowest three
  bins, where a broken closure piles particles up, within tolerance). Both directions, both
  regimes, with the production closure imported and the no-floor control beside it. Passes with 0
  turnovers ([sixth pass](../history/pass-6.md), [ninth pass](../history/pass-9.md)). What it
  cannot see: whether `σ_w` has the right magnitude.
...
`bin/test_ml_data.py` (the loader refuses the test split; every test-split read in the audit log
carries `allow_test`; the cone masks, the normalisation and the split are the file's own),
`bin/test_ml_model.py` (a zero residual reproduces Kljun to 7.5e-8; every parameter has a
gradient; the metrics reproduce the production functions), `bin/test_cfm.py` (26 checks: the
interpolant, the CRPS estimator, the test guard, and that `ml/`, `results/ml/final/` and the
first CFM run are unchanged by hash).

...
| 6 | FastEddy: 30 minutes of adjustment under the case's own forcing, then a 30-minute sampling window passed to the LPDM in RAM | `bin/run_window.sh`, patch 0005 |
| 7 | the backward LPDM: particles released at the receptor through the window and followed back to their touchdowns. The signed footprint on the LES's own 122 × 122 cells | `bin/stage5_footprint.py`, `lpdm/` |
...
Details are in [configuration](../les/configuration.md), [seed library](../les/seed-library.md),
[case generation](../les/case-generation.md) and [LPDM and footprint](../les/lpdm-and-footprint.md).

The pipeline is validated by gates that check the artifacts each stage produces, never exit
codes or the configuration passed in ([gates and diagnostics](../les/gates-and-diagnostics.md)).
...
1366 cases, one per day over 2021–2026. The hour is drawn at random from each day and screened
for an unstable, quasi-stationary boundary layer between 300 and 1250 m deep. The split is by
calendar year: 2024 is validation and 2025 is test. There are two HDF5 files with identical
layout. `corpus_raw.h5` is the corpus as produced. `corpus_cone.h5` has the periodic wraparound
cropped by a wind-aligned cone whose one parameter was measured from an empty valley in the mass
distribution. Only about 15% of records have the array in view. See
[the dataset](../corpus/dataset.md).
...
- **CFM**: conditional flow matching from the noised Kljun prior to the LES target, with a
  2.95 M-parameter U-Net velocity field. Its sample mean ties the FNO. Its samples give the
  realisation spread, so the array share comes with an error bar.

See [targets and architecture](../emulator/targets-and-architecture.md), [training](../emulator/training.md),
[results](../emulator/results.md) and [calibration](../emulator/calibration.md).
...
the input channel, the anchor of the residual, the baseline every metric is scored against and
the source of the `σ_y` that defines the cone. The project's earlier reimplementation was
1.25× too wide in `σ_y` in the near-neutral regime and was retired.
...
Every record stores the image commit that made it, the seed and rotation it restarted from,
the seed's stationarity verdict, the grid, the closure configuration and the FFP version. The
corpus image is pinned by digest. FastEddy is pinned by release tag plus a patch series with a
checksummed manifest. The emulator's environment is pinned by `ml/environment.yml`. Every
corpus read by the emulator code is logged, so the single audited read of the test split is on
record.

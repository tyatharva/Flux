...
limit (docs/les/configuration.md). Between the two the model exits 0, prints no warning, and silently
...
def check(path):
    ...
    # is exactly what a rho-skipped ioLPDMmode dump looked like (docs/reference/fasteddy-traps.md #1).
    ...
    if nbad or not np.isfinite(ratio):
        return "FAIL", (f"  FAIL: NON-FINITE w in {path}\n"
                        ...
                        f"        inf. See docs/reference/fasteddy-traps.md #1.")
    if ww[1] < WW_FLOOR:
    ...
    if ratio >= LIMIT:
        return "FAIL", (f"  FAIL: near-surface w variance ratio {ratio:.2f} >= {LIMIT} in {path}\n"
                        ...
                        f"        grid-scale acoustic noise. Lower dt; see docs/les/configuration.md.")
